...
namespace RespirationsComment
{
    public class DurationComment
    {
        ...
        private static readonly float MINIMUM_LATERAL_DURATION = 20.0f; //used for position effect 
        ...
        private float mSupineSleepTime;
        private float mRemSleepTime;
        private float mTotalSleepTime;

        public static float MinimumLateralDuration
        {
            get { return MINIMUM_LATERAL_DURATION; }
        }
        ...
        private void initializeSupineDuration()
        {
            if (mSupineSleepTime == ZERO)
                ...
                if (mSupineSleepTime <= MINIMUM_SUPINE_DURATION)
                    mSupineDuration = Duration.Limited;
                else
                    if (mSupineSleepTime > MINIMUM_SUPINE_DURATION)
                        mSupineDuration = Duration.Normal;
                    else
                        System.Diagnostics.Debug.Assert(false, "No value found.");
        }
        private void initializeRemDuration()
        {
            if (mRemSleepTime == ZERO)
                ...
                if (mRemSleepTime <= MINIMUM_REM_DURATION)
        ...
        private void getResults(String path)
        {
            ...
            for (int i = 0; i < durationResults.Length; i++)
            {
 ...
 /*
        ...
        private string mDurationResult;
        public string DurationResult
        {
            get { return mDurationResult; }

        }
        private void getDurationComment()
        {
            int index = durationTable[(int)mSleepDuration, (int)mSupineDuration, (int)mRemDuration];
            mDurationResult = durationResults[index];
        }
