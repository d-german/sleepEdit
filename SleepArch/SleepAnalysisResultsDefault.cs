...
namespace SleepArch
{
    ...
    {
        const int numStage1 = 2; // Number of possible values for stage 1
        const int numStageSWS = 4; // Number of possible values for slow wave sleep.
        const int numStageREM = 4; // Number of values for REM sleep.
        const int numResults = 32; // 2*4*4 = 32


        static int[, ,] resultsTable = new int[numStage1, numStageSWS, numStageREM];
        static string[] results = new string[numResults];
        static string[] Labels = new string[numResults];
        ...
        {
            int index = resultsTable[(int)stage1, (int)slowWaveSleep, (int)REMsleep];
            return results[index];
        }

        public override string GetResults(Stage1 stage1, Stage2 s2, SWS sws, REM rem)
        {
            System.Diagnostics.Debug.Assert(false, "This is not used for default");
            return "";
        }

        public void printDefaultResults()
        {
            for (int n1 = 0; n1 < numStage1; n1++)
            {
                for (int n3 = 0; n3 < numStageSWS; n3++)
                {
                    for (int rem = 0; rem < numStageREM; rem++)
                    {
                        System.Console.WriteLine(results[resultsTable[n1, n3, rem]]);
                    }
                }
            }
